...
/// Loads fonts and icons to ensure proper appearance in golden tests.
///
/// How to use:
/// 1. Create a `flutter_test_config.dart` file.
/// 2. Insert `await loadFonts();` inside the `testExecutable` function.
///
/// Important: Make sure your package includes all necessary fonts as assets for this to function
/// correctly.
...
class _FontData {
  ...
  factory _FontData.fromJson(Map<String, dynamic> json) {
    return _FontData(
      ...
      fonts: (json['fonts'] as List).map((font) => _FontType.fromJson(font)).toList(),
    );
  }
  final String family;
